...
** 关于用VC2013编译时的问题
** 由于原作者misakamm在 ege.h 中 使用#pragma指定了链接库，并且将所有版本号_MSC_VER
** 大于1700的VC编译器全部指定为使用 graphics12.lib，导致编译失败。
...
*/
/* 20150203 by cyd AT bupt dot edu dot cn
** 从VS2013开始，库文件名中的最后一个字符“d”代表是debug版本，而非动态库(dll)版本。
** 从VS2008开始，方案中的所有项目都有三类属性：Release（静态库链接）、ReleaseD（动态库
**     链接）、Debug（含有调试信息）。ege的作者想以库文件名中最后一个字符“d”来表示
**     ReleaseD版本
*/
#if !defined(_GRAPH_LIB_BUILD_) && !defined(_GRAPH_NO_LIB_)
	#ifdef _MSC_VER
		#ifdef _WIN64
			#if (_MSC_VER > 1900)  //VS2016及之后的版本，还是不加年号了
				#if defined(_DEBUG)
					#pragma comment(lib,"graphics64d.lib")
				#else
				...
				#endif
			#else										  //VC2005及更早的版本，包括VC6，就与VS2016一样待遇吧
				#pragma comment(lib,"graphics.lib")
			#endif
		#endif
		#if _MSC_VER >= 1700
...
int EGEAPI putimage_alpha(
	PIMAGE imgdest,         // handle to dest
	PIMAGE imgsrc,          // handle to source
	double alpha,				    // alpha, from 0.0 to 1.0
	int nXOriginDest,       // x-coord of destination upper-left corner
	int nYOriginDest,       // y-coord of destination upper-left corner
	int nXOriginSrc = 0,    // x-coord of source upper-left corner
	int nYOriginSrc = 0,    // y-coord of source upper-left corner
...
*/
//以下5个函数，是为了帮助C语言初学者解决数字字符输入解析的问题
//Programmer: 周斯达 (blacketzsd AT qq dot com) 2013/06/01

//限制输入为整形的输入框，可改变 对话框提示文字；返回输入的整数
int getInteger(LPCSTR text = "请输入一个整数，然后按回车键确认");

//限制输入为浮点数的输入框，可改变 对话框提示文字；返回输入的浮点数
double getDouble(LPCSTR text = "请输入一个浮点数，然后按回车键确认");

//限制输入内容长度的字符串输入框，可改变 对话框提示文字；输入的字符串存放在 str[]中
void getString(char str[], int length, LPCSTR text = "请输入一个字符串，然后按回车键确认");

//限制输入为字符的输入框，可改变 对话框提示文字；返回输入的字符
char getChar(LPCSTR text = "请输入一个ASCII字符");
...
//title:    对话框的标题
void getCoords(int coords[], unsigned int pairs, LPCSTR title = "输入点的坐标");
