...
private:
  ...
  void attack(int& target_row, int& target_col);                            //备用贪心算法
  void defend(int& target_row, int& target_col);                            //备用贪心算法

  void show_score_tabel(GridScore score[][Chess::SIZE]);                    //调试输出方法

  double calc_chess_score(Chess::PieceType self_type, int min_max_flag);
  void calc_score_table();
  double DFS(int nth_step, double ex_value, int min_max_flag, int& target_row, int& target_col);        //博弈树深搜算法
