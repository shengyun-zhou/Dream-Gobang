	...
	)
	{
		...
		if (img) {
			...
			for (y = 0; y<nHeightSrc; ++y) {
				for (x = 0; x<nWidthSrc; ++x, ++psp, ++pdp) {
					DWORD alpha2 = *psp >> 24;
					alpha2 = (DWORD)((double)alpha2 * alpha);
					DWORD sa = alpha2 + 1, da = 0xFF - alpha2;
					DWORD d = *pdp, s = *psp;
					d = ((d & 0xFF00FF)*da & 0xFF00FF00) | ((d & 0xFF00)*da >> 16 << 16);
					s = ((s & 0xFF00FF)*sa & 0xFF00FF00) | ((s & 0xFF00)*sa >> 16 << 16);
					if (alpha2 > 0)
...
putimage_alpha(
	...
	{
		return imgsrc->putimage_alpha(imgdest, alpha, nXOriginDest, nYOriginDest, nXOriginSrc, nYOriginSrc, nWidthSrc, nHeightSrc);
	}
